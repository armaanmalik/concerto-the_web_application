<?php 
										...
										// Start the session
											...
											/*if (($_SESSION['timeout'] + (12 * 60)) < time()) 
											{
...
<body>
	...
	<section >	
								...
								<?php 
										...
										//	$result = $orderStmt->get_result();
											...
											while ($orderStmt->fetch()) 
												...
												echo"<form name='form1' action = 'undo.php' method='post'>";
												echo "<tr id ='x1'>";
												echo "<td>$gname</td>";
												echo "<td>$gsubcategory</td>";
												echo "<td><input type=\"radio\" name=\"gname1[]\" value=\"$gname:$gsubcategory\"></input></td><td><input type='submit' name = 'unlike' value='Unlike'></input></td>";
												echo "<br>";
												echo "</tr>";
												}
						...
						<div id = "band">
								...
								<?php 	
											//Preparing and executing the query in database
											$orderQuery = 	"Select distinct b.bid,b.bname, p.gname from fan f, band b, plays p  where f.bid=b.bid and b.bid=p.bid and uid= ? ";
									...
									while ($orderStmt->fetch()) 
										...
										echo "<td>$gname</td>";
										echo "<td><input type=\"radio\" name=\"bname[]\" value=\"$bid:$bname\"></input></td><td><input type='submit' name = 'unfan' value='Unfollow'></input></td>";
								...
								<?php 
										...
										//	$result = $orderStmt->get_result();
											...
											while ($orderStmt->fetch()) 
												...
												echo"<form name='form' action ='undo.php' method='post'>";
												echo "<tr id='x1'>";
												echo "<td><a id='x1' href='userview.php?id=$followingid'>$uname</a></td>";
												echo "<td><input type=\"radio\" name=\"username[]\" value=\"$followingid:$uname\"></input></td>
												<td><input type='submit' name = 'unfollow1' value='Unfollow'></input></td>";
						...
						<div id= "recommend_concert">
								...
								<?php 
										//Following User's Concert
											...
											//Preparing and executing the query in database
											$orderQuery = 	"Select c.cid,c.cname,b.bname,v.vname,v.vcity,c.ctime,c.tktprice,c.tktlink,c.avail
															from concert c,venue v,band b, user a 
															where c.bid=b.bid and c.vid=v.vid and a.uid  in ((Select distinct followingid 	from follow where followerid= ?) or(Select distinct followerid 	from follow where followingid= ?) ) ";
								...
								<?php 
										//Following User's Concert
											...
											$orderQuery = 	"Select c.cid,c.cname,b.bname,v.vname,v.vcity,c.ctime,c.tktprice,c.tktlink,c.avail
															...
															where c.bid=b.bid and c.vid=v.vid and c.cposttime> u.lastaccess and u.uid=? ";
